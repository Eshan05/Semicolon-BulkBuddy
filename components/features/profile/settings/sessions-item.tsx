            ...
            <div className="grid gap-2">
              {sessions ? (
                (() => {
                  ...
                  sessions
                    .filter((s: SessionItem) => s.userAgent)
                    .forEach((s: SessionItem) => {
                      const os =
                        new UAParser(s.userAgent || "").getOS().name || "Unknown";
                      let key = "Other";
                    ...
                    <div className="w-full mt-2 space-y-4">
                      {entries.map(([key, list]) => (
                        <div key={key} className="w-full">
                          <div className="flex items-center justify-between gap-3 px-2 py-2 rounded-md bg-muted/50 w-full mb-2">
                          ...
                          <div className="px-2 py-1">
                            <div className="grid gap-2">
                              {list.map((s: SessionItem) => {
                                ...
                                const deviceName =
                                  device.model || device.vendor || osName;
                                const location =
                                  s.location ||
                                  s.geo ||
                                  s.city ||
                                ...
                                return (
                                  <div
                                    key={s.id}
                                    className="flex items-center justify-between p-2 rounded-md hover:bg-gray-50 dark:hover:bg-slate-800"
                                  >
                                    ...
                                    <div>
                                      <Button
                                        size="icon"
                                        variant={
                                          isCurrent ? "outline" : "ghost"
                                        }
                                        className="h-8 w-8 text-destructive hover:text-destructive hover:bg-destructive/10"
                                        onClick={async () => {
                                          setIsTerminating(s.id);
                                          const r = await client.revokeSession({
                                            token: s.token,
                                          });
                                          if (r?.error)
